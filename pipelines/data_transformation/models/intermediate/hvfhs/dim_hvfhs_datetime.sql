...
/*
  Dimensión: Fecha y hora para HVFHS
  Granularidad horaria.
  HVFHS tiene 3 timestamps (request, pickup, dropoff) — se incluyen los 3.
*/

WITH request_hours AS (
    SELECT DISTINCT DATE_TRUNC('hour', request_datetime) AS datetime_hour
...
all_hours AS (
    SELECT datetime_hour FROM request_hours
    UNION
