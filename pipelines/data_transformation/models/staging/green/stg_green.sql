...
normalized_columns AS (
    SELECT 
        {% if var("anio") | int == 2024 %}
            ...
            CAST(trip_type AS INTEGER) AS trip_typ
        ...
        {% elif var("anio") | int >= 2025 %}
            ...
            COALESCE(CAST(cbd_congestion_fee AS DOUBLE),0) AS true_total_amt,
            ...
            COALESCE(CAST(congestion_surcharge AS DOUBLE),0) +
            COALESCE(CAST(improvement_surcharge AS DOUBLE),0))+
            COALESCE(CAST(cbd_congestion_fee AS DOUBLE),0)  AS comparation_total_amt,
